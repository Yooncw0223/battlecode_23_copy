    ...
    static void runHeadquarters(RobotController rc) throws GameActionException {
        ...
        //for (int i = 0; i < 5; i++) {
            try {
                ...
                if ((RobotPlayer.turnCount < 300 || rc.getNumAnchors(Anchor.STANDARD) == 1 || rc.getResourceAmount(ResourceType.ADAMANTIUM) >= 150) && rc.canBuildRobot(RobotType.CARRIER, loc)) {
                    rc.setIndicatorString("Trying to build a resource carrier");
                    rc.buildRobot(RobotType.CARRIER, loc);
                }

                loc = it.next();
                if ((RobotPlayer.turnCount < 300 || rc.getNumAnchors(Anchor.STANDARD) == 1 || rc.getResourceAmount(ResourceType.MANA) >= 145)
