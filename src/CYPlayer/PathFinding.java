...
public class PathFinding {
    ...
    public static boolean move(RobotController rc, MapLocation destination) throws GameActionException {
        ...
        boolean reset = PathFinding.dest == null || !dest.getLocation().equals(destination);
        if (reset) {
        ...
        if (progress == 3) {
            ...
            if (progressIndex >= 0 && progressIndex <= nav.size() - 1) {
                rc.move(nav.get(progressIndex).opposite());
                progressIndex -= 1;
            }
        ...
        if (current.equals(PathFinding.dest)) {
            return true; 
        }
